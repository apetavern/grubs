	...
	public override void FireInstant()
	{
		...
		if ( Game.IsServer )
		{
			foreach ( var (grub, dir) in grubsHit )
			{
				...
				grub.TakeDamage( new DamageInfo { Attacker = Grub, Damage = Damage, Position = grub.Position, Weapon = Weapon }.WithTag( Tag.Melee ) );
			}
		}
	...
	private Dictionary<Grub, Vector3> GetGrubsInSwing()
	{
		...
		foreach ( var trace in trs )
		{
			if ( trace.Entity is Grub grub )
				...
				gadget.TakeDamage( new DamageInfo { Attacker = Grub, Damage = Damage, Position = gadget.Position }.WithTag( Tag.Melee ) );
		}
