...
public sealed class PremadeTerrain
{
	...
	public static PremadeTerrain Deserialize( BinaryReader reader )
	{
		...
		return new PremadeTerrain( version, width, height, terrainGrid, settings );
	}
	...
	public static void Serialize( BinaryWriter writer, TerrainMap map, bool preserveSettings )
	{
		...
		var currentValue = map.TerrainGrid[0, 0];
		var valueChanges = new List<(int, int)>();
		for ( var y = 0; y < map.TerrainGrid.GetLength( 1 ); y++ )
		{
			for ( var x = 0; x < map.TerrainGrid.GetLength( 0 ); x++ )
			{
				if ( currentValue == map.TerrainGrid[x, y] )
					continue;

				valueChanges.Add( (x, y) );
				currentValue = !currentValue;
			}
		}
		...
		for ( var i = 0; i < valueChanges.Count; i++ )
		{
			writer.Write( valueChanges[i].Item1 );
			writer.Write( valueChanges[i].Item2 );
		}
	}
