...
PS
{
	...
	float4 g_vTint_Colour < UiType( Color ); UiGroup( "Tint,0/,0/2" ); Default4( 0.41, 0.22, 0.11, 1.00 ); >;
	...
	bool g_bTintGradientEnabled < UiGroup( "Tint,0/,0/0" ); Default( 1 ); >;
	...
	float4 MainPs( PixelInput i ) : SV_Target0
	{
		...
		float4 local26 = g_bTintGradientEnabled ? local25 : local3;
		float4 local27 = saturate( lerp( local26, SoftLight_blend( local26, local4 ), 0.5 ) );
		float4 local28 = g_vScorchTint_Colour;
		float4 local29 = Tex2DS( g_tScorchColour, g_sSampler0, local2 );
		float4 local30 = local28 * local29;
		float4 local31 = g_vScorchLayer_Params;
		float4 local32 = float4( local31.r, local31.g, 0, 0 );
		float3 local33 = i.vPositionOs;
		float3 local34 = float3( local31.b, local31.b, local31.b ) * local33;
		float4 local35 = local32 + float4( local34, 0 );
		float4 local36 = Tex2DS( g_tScorchLayer, g_sSampler1, local35.xy );
		float local37 = local36.r - 0.5;
		float local38 = local37 * local31.a;
		float local39 = g_flScorchBlendDistance;
		float local40 = local31.a / local39;
		float local41 = local38 * local40;
		float local42 = local41 * -0.5;
		float4 local43 = Tex2DS( g_tScorchBlendMask, g_sSampler0, local2 );
		float local44 = local43.r - 0.5;
		float local45 = local44 * 32;
		float local46 = local42 - local45;
		float local47 = max( local46, 0 );
		float local48 = min( local47, 1 );
		float4 local49 = lerp( local27, local30, local48 );
		float4 local50 = Tex2DS( g_tNormal, g_sSampler0, local2 );
		float4 local51 = Tex2DS( g_tScorchNormal, g_sSampler0, local2 );
		float4 local52 = lerp( local50, local51, local48 );
		float3 local53 = TransformNormal( i, DecodeNormal( local52.xyz ) );
		float4 local54 = Tex2DS( g_tRough, g_sSampler0, local2 );
		float4 local55 = Tex2DS( g_tScorchRough, g_sSampler0, local2 );
		float local56 = lerp( local54.a, local55.r, local48 );
		float4 local57 = Tex2DS( g_tAO, g_sSampler0, local2 );
		float4 local58 = Tex2DS( g_tScorchAO, g_sSampler0, local2 );
		float local59 = lerp( local57.r, local58.r, local48 );
		
		m.Albedo = local49.xyz;
		m.Opacity = 1;
		m.Normal = local53;
		m.Roughness = local56;
		m.Metalness = 0;
		m.AmbientOcclusion = local59;
